...
p1 <- 
  ...
  scale_x_continuous(limits = c(1890, 2024), 
                     ...
                     expand = c(0, 0)) +
  geom_text(
    data = dplyr::tibble(year = c(1962.5),
                         votes = c(32),
                         label = c("JAMES G. HAGAN")),
    mapping = aes(x = year, y = votes, label = label),
    inherit.aes = FALSE, 
...
ggsave(filename = "06-banner-image/figures-tables/fig3.pdf",
       p1, 
       width = 1500, height = 500, units = "px")

pdftools::pdf_convert(pdf = "06-banner-image/figures-tables/fig3.pdf",
                      filenames = "06-banner-image/figures-tables/fig3.jpeg",
                      format = "jpeg", dpi = 3000)
